...
d3.tsv("data.tsv").then(function(incomingData){
  ...
  let force = d3.forceSimulation(reducedData)
    ...
    .on("end", function(){
      reducedData.forEach(node=>{
        // the simulation has calculated where the circles should be
        // the positions that it calulcated are stored in node.x and node.y
        // let's make a copy of those positions for future use:
        node.yearx = node.x;
        node.yeary = node.y;
        // here i am manually setting the positions to be where the NEXT simulaton starts
        // note this is the country position, same as the force inside calcCountryPos()
        // i set this to be the position so that the next simulation starts calculating fro there
  ...
  function calcCountryPos(){
    ...
    force = d3.forceSimulation(reducedData)
      ...
      .on("end", function(){
        reducedData.forEach(node=>{
          //after country positions are calculated we make a copy of the results
          node.countryx = node.x;
          node.countryy = node.y;
          // and prepare the x and y value for the next simiulation which will 
          // start off fro their genre position:
