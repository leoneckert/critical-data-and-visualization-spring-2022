...
d3.json("monarchs.json").then(function(incomingData){
  ...
  const posInterpolate = d3.interpolate({
    g1: [
      "red", 
      "blue"
    ]}, 
  ...
  // function showGraph1(){
    ...
    let datagroups = graphicGroup.selectAll(".datagroup").data(data).enter().append("g")
      .attr("class", "datagroup")
      .attr("transform", function(d, i){
        // building a own interpolation function for every single datapoint and
        // appending it to that datapoint for future access:
        // from here: https://github.com/d3/d3-interpolate/blob/main/README.md
        d.posInterpolate = d3.interpolate([
            g1xScale(d.date), 
            g1yScale(d.type)+g1yScale.bandwidth()/2
          ], 
          [
            g2xScale(d.type)+g2xScale.bandwidth()/2, 
            g2yScale(d.countInType)
          ]
        );
        // this function expects values between 0 and 1 and returns
        // the interpolated position between g1 and g2 of any datapoint
        // right here we give the function "0" to get back the position for the g1
        // passing "1" into the function will give the position on g2. anything
        // in between show positions between the two graphs
        let x = d.posInterpolate(0)[0]
        let y = d.posInterpolate(0)[1]
  ...
  //       let x = d.posInterpolate(1)[0]
  //       let y = d.posInterpolate(1)[1]
  //       // let bandHeight = g1yScale.bandwidth(); // returns width (in this height) of the band scale
  //       return "translate("+x+", "+y+")"
  //     })
  //   ;
  ...
  enterView({
    ...
    progress: function(el, progress) {
      ...
      datagroups
        .attr("transform", function(d, i){
          let x = d.posInterpolate(progress)[0]
          let y = d.posInterpolate(progress)[1]
          // let bandHeight = g1yScale.bandwidth(); // returns width (in this height) of the band scale
          return "translate("+x+", "+y+")"
        })
      ;
